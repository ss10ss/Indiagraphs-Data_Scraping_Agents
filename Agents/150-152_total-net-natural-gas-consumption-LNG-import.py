...
# All three datasets are published on the same PPAC table. Each config keeps
# the selector behavior of its original standalone scraper.
DATASET_CONFIGS = [
    {
        "dataset_id": 150,
        "dataset_name": "Total natural gas consumption",
        "row_description": "Total Consumption (Net Production + LNG import)",
        "row_xpath": "//tr[.//b[contains(text(), 'Total Consumption (Net Production + LNG import)')] or .//td[contains(text(), 'Total Consumption (Net Production + LNG import)')]]",
    },
    {
        "dataset_id": 151,
        "dataset_name": "Net natural gas consumption",
        "row_description": "Net Production",
        "row_xpath": "//tr[.//b[normalize-space(text())='Net Production'] or .//td[normalize-space(text())='Net Production']]",
    },
    {
        "dataset_id": 152,
        "dataset_name": "LNG import",
        "row_description": "LNG import**",
        "row_xpath": "//tr[.//b[normalize-space(text())='LNG import**'] or .//td[normalize-space(text())='LNG import**']]",
    },
]
# =====================================================================
...
def navigate_to_table(driver, wait):
    ...
    """
    print("Opening page...")
    driver.get("https://ppac.gov.in/natural-gas/consumption")
...
def read_month_headers(driver):
    print("Reading month headers...")
    header_cells = driver.find_elements(
        By.XPATH,
        "//table[.//th[contains(text(), 'April')]]//tr[1]/th",
    )
    month_headers = []
...
def scrape_dataset(driver, config, fiscal_year_start, month_headers):
    """
    Extract one configured PPAC row into period/value dictionaries.
    Returns (scraped_rows, extraction_failures).
    """
    dataset_id = config["dataset_id"]
    row_description = config["row_description"]

    print(f"\n{'=' * 72}")
    print(f"Dataset {dataset_id}: {config['dataset_name']}")
    print(f"Locating the '{row_description}' row...")
    print(f"{'=' * 72}")

    try:
        data_row = driver.find_element(By.XPATH, config["row_xpath"])
        value_cells = data_row.find_elements(By.XPATH, "./td[position() > 1]")
    except Exception as e:
        return [], [{"period_label": "ROW_NOT_FOUND", "error": str(e)}]
...
def insert_new_dataset_rows(config, scraped_rows):
        ...
        return 0, [
            {
                ...
                "error": "Not a single month column had data - the row selector may have changed.",
            }
        ]
    ...
    for item in scraped_rows:
        try:
            ...
            print(f"\nProcessing dataset {dataset_id} -> Month: {period_label}, Value: {value}")
            ...
            print(f"'{period_label}' is absent from both tables. Inserting into '{DRAFT_TABLE}'...")
...
try:
    # ---- Run the full navigation flow once, with retries ----
    ...
    month_headers = read_month_headers(driver)

    total_extracted = 0
    total_inserted = 0
    all_failures = []

    # ---- Extract and insert all three datasets from the one loaded page ----
    for config in DATASET_CONFIGS:
        scraped_rows, extraction_failures = scrape_dataset(
            driver,
            config,
            fiscal_year_start,
            month_headers,
        )
        inserted_count, processing_failures = insert_new_dataset_rows(config, scraped_rows)
